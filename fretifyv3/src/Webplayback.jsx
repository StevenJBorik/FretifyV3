    ...
    window.onSpotifyWebPlaybackSDKReady = () => {
      if (!isSDKInitialized.current) {
        ...
        player.addListener('player_state_changed', async (state) => {
          ...
          try {
            const response = await fetch(`http://fretifyv3-server:5000/audio-analysis/${trackId}`);
          ...
          try {
            const response = await fetch('http://fretifyv3-server:5000/predict-scale-change', {
              method: 'POST',
              headers: {
